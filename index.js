...
let generateSquares = () => {
    ...
    for(i=0; i<10; i++) {
        squares.push('<div class="grid-item" data-hit="1">1</div>')
    }
 // replace 12. for every hit we are putting a div into the array with data-hit 0
    for(i=0; i<12; i++) {
        squares.push('<div class="grid-item" data-hit="0" >0</div>')
    }
    //shuffle the array
    let shuffle = () => squares.map(a => ({ sort: Math.random(), value: a })).sort((a, b) => a.sort - b.sort).map(a => a.value)
